...
\begin{definition}
  Alternative $a$ is a \emph{Condorcet winner} if for any alernative $b$ a majority of individuals prefer $a$ over $b$.
\end{definition}
\begin{itemize}
...
\fr{\ft{Social choice functions}
  \begin{itemize}
  \item sometimes it is enough to choose one alternative (instead of giving whole welfare ranking)
...
\framebreak
  \begin{ex}[continued]
    If we restrict preferences, the result does not necessarily hold. Show that with single-peaked preferences, truthful reporting is a weakly dominant strategy. Conlude that the result stated above does not hold on single peaked preference domains.
  \end{ex}
}
