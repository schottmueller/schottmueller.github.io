...
\begin{titlepage}
	...
	\vfill
        ...
        \end{tabular}
	%supervisor:\par
	%Prof. Dr. \textsc{C. Schottmüller}\par
        %\vspace*{0.25cm}
        %second reader:\par
        %\textsc{M. Gramb}
...
Also $f(x^*)\leq x^*$: if not, i.e. $f(x^*)>x^*$, then $x^*+\varepsilon  < f(x^*+\varepsilon )$ for some $x^* +\varepsilon \in(x^*,f(x^*) ]\cap [0,1]$ because $f$ is increasing which contradicts the definition of $x^*$. Hence, $x^*=f(x^*)$.\qed
