...
\begin{equation}
  ...
  f(x)=x.
\end{equation}
This equation can be referenced as equation (\ref{eq:fixedPoint}).

To get several equations in a row with aligned equality signs you can use the align environment where \verb|&| indicates the position that should be aligned and \verb|\\| indicates a linebreak, i.e. the sart of the next equation:
...
Each equation can be referenced on it sown like equation (\ref{eq:f}) or equation (\ref{eq:g}) here.
...
\par
Another useful hint concerns paragraph formatting. If you want to start a new paragraph, you either have to leave a blank line in your code (hit Enter twice) or you use the command \verb|\par|. A normal linebreak occurs if you use \verb|\\| but please do not use this to start a new paragraph as does not indent the first line of the next paragraph which is considered bad style. (I do it here nevertheless, so you see the difference.)\\
...
$f(x^*)\geq x^*$:  if $x^*\in A$, this holds by the the definition of $A$, see equation (\ref{eq:A}). But $x^*\in A$ has to hold by teh definition of $x^*$ as otherwise $f$ would have to discontinously drop downward at $x^*$ contradicting that $f$ is increasing.
